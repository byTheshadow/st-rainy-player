...
async function subApiGenerate({ ordered_prompts, max_chat_history = 20 }) {
    ...
    try {
        ...
        // 提取 <yuezhang>...</yuezhang> 内容
        function extractYuezhangContent(chatHistory) {
            const results = [];
            const regex = /<yuezhang>([\s\S]*?)<\/yuezhang>/g;
        ...
        try {
            if (typeof SillyTavern !== 'undefined' && SillyTavern.getContext) {
                ...
                if (ctx.chat && Array.isArray(ctx.chat)) {
                    ...
                    yuezhangHistory = extractYuezhangContent(recentChat);
                }
            }
        } catch (e) {
            ...
            if (typeof item === 'string') {
                ...
                switch (item) {
                    case 'char_description':
                        if (charDescription) {
                            messages.push({ role: 'user', content: `[角色描述]\n${charDescription}` });
                        }
                        break;

                    case 'char_personality':
                        if (charPersonality) {
                            messages.push({ role: 'user', content: `[角色性格]\n${charPersonality}` });
                        }
                        break;

                    case 'chat_history':
                        messages.push(...yuezhangHistory);
                        break;

                    case 'world_info_before':
                        if (worldInfoBefore) {
                            messages.push({ role: 'user', content: `[世界信息-前置]\n${worldInfoBefore}` });
                        }
                        break;

                    case 'world_info_after':
                        if (worldInfoAfter) {
                            messages.push({ role: 'user', content: `[世界信息-后置]\n${worldInfoAfter}` });
                        }
                        break;
        ...
        console.log('[RainyPlayer] messages =', messages);
